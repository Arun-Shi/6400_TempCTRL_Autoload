...
u8 DS18B20_ALL_Reset(void)
{
	...
	__DS_ALL_Pin_Get1Byte(DS_Status)
...
void DS18B20_ALL_Read_Temp_float(void)
{
	...
	{
		data=(Data_hight[j]<< 8 )+ Data_Low[j];
		DS_ALL_Arr[j].Tempdata= data * 0.0625;
	}
}
...
short DS18B20_Read_Temp_Short(IO_Pos Pin)
{
	u8 Data_Low,Data_hight;
